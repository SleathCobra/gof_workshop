...
| GOF2 BIN-family boundaries | Local cross-platform filenames and byte layouts; DeepOpen/gof2hd-decomp consumer names as behavioral corroboration | All 136 discovered files classify, parse, and reproduce byte-for-byte unchanged; generated boundary/truncation fixtures exercise every exposed parser shape | Independent sample comparison and bounded cursor implementation; unresolved records remain opaque |
| BIN editable scalar locations | Repeated record lengths and endianness across PC/Android/iOS/macOS samples | Size-stable edits alter only the recorded byte range, reparse successfully, and undo/recovery reproduce the prior bytes | Structural sample analysis; semantic labels are limited to confirmed/high-confidence fields |
| glTF transform-animation import | Public glTF 2.0 accessor, sampler, channel, and quaternion definitions | Synthetic animation imports, converts to scalar AEM tracks, writes/reparses, and evaluates; Blender 5.1 headless round trip preserved stable IDs and produced playable output | Public specification plus independent conversion tests; STEP/CUBICSPLINE, rigs, and morphs are rejected |
| Browser WebGL rendering behavior | WebGL 2 / GLSL ES 3.00 public APIs | Real Brave and Edge smoke runs rendered selected normalized scenes, reacted to camera input, and rebuilt resources after forced context loss | Independently authored focused renderer; no third-party JavaScript renderer code or package |
